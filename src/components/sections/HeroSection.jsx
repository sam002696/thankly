...
const HeroSection = ({ onCreateCard }) => (
  <header className="relative min-h-screen flex items-center pt-24 overflow-hidden">
    ...
    <div className="absolute inset-0 z-0 pointer-events-none">
      <svg className="absolute top-32 left-12 w-16 h-16 text-brand opacity-80 rotate-12" viewBox="0 0 100 100" fill="none" stroke="currentColor" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round">
        <path d="M50 10 L60 40 L90 40 L65 60 L75 90 L50 70 L25 90 L35 60 L10 40 L40 40 Z" fill="transparent" />
      </svg>
      <svg className="absolute top-24 right-32 w-48 h-24 text-cream opacity-90" viewBox="0 0 200 100" fill="none" stroke="currentColor" strokeWidth="8" strokeLinecap="round">
        <path d="M10,50 Q30,10 50,50 T90,50 T130,50 T170,50 T210,50" style={{ strokeDasharray: 1000, strokeDashoffset: 0 }} />
      </svg>
      <svg className="absolute bottom-24 left-24 w-12 h-12 text-accent opacity-80 -rotate-12" viewBox="0 0 100 100" fill="none" stroke="currentColor" strokeWidth="6" strokeLinecap="round" strokeLinejoin="round">
        <path d="M50 85 L15 50 A 20 20 0 0 1 50 20 A 20 20 0 0 1 85 50 Z" />
      </svg>
      <svg className="absolute top-1/2 left-[45%] w-10 h-10 text-brand opacity-60 rotate-45" viewBox="0 0 100 100" fill="none" stroke="currentColor" strokeWidth="5" strokeLinecap="round" strokeLinejoin="round">
    ...
    <div className="container mx-auto px-8 relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
      <div className="max-w-xl pl-4 md:pl-8">
        <div className="inline-block mb-4 relative">
          <span style={{ fontFamily: 'Caveat, cursive', transform: 'rotate(-2deg)', display: 'inline-block' }} className="text-3xl md:text-4xl text-success bg-white/80 px-4 py-1 rounded-lg border-2 border-dashed border-success shadow-sm">
            Spread joy daily ✨
          </span>
          <svg className="absolute -top-4 -right-6 w-8 h-8 text-brand rotate-12" viewBox="0 0 24 24" fill="currentColor">
            <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z" />
          </svg>
        </div>

        <h1 style={{ fontFamily: 'Caveat, cursive' }} className="text-7xl md:text-8xl leading-[0.9] text-ink mb-6 drop-shadow-sm">
          the Importance of <br />
          <span className="text-8xl md:text-[140px] text-brand font-bold uppercase tracking-tight block mt-2">GRATITUDE</span>
        </h1>

        <p className="text-xl text-ink/80 mb-10 font-medium leading-relaxed max-w-md">
        ...
        <div className="flex flex-col sm:flex-row gap-6">
          <Button variant="primary" size="lg" onClick={onCreateCard}>
      ...
      <div className="relative h-150 w-full flex items-center justify-center">
        <div className="relative w-full max-w-md aspect-square">
          ...
          <div className="absolute bottom-10 left-10 w-32 h-32 bg-success rounded-full mix-blend-multiply opacity-60 blur-xl"></div>

          <div className="absolute inset-0 flex items-center justify-center">
            <div className="relative z-10 w-64 h-80">
              <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-48 h-40 bg-[#8CB388] rounded-t-[100px] border-4 border-ink"></div>
              <div className="absolute top-10 left-1/2 -translate-x-1/2 w-32 h-32 bg-[#FFD1B3] rounded-full border-4 border-ink flex items-center justify-center overflow-hidden">
                <svg viewBox="0 0 100 100" className="w-full h-full opacity-80">
                  ...
                  <circle cx="25" cy="55" r="8" fill="#ef4444" opacity="0.4" />
                  <circle cx="75" cy="55" r="8" fill="#ef4444" opacity="0.4" />
              ...
              <div className="absolute bottom-12 left-1/2 -translate-x-1/2 z-30 transform hover:scale-110 transition-transform cursor-pointer">
                <svg className="w-28 h-28 text-brand drop-shadow-xl" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z" />
                </svg>
              </div>
            </div>
